...
class Window1:
      def __init__(self, root):
        self.root=root
        self.root.title(" Medical Store")
        self.root.geometry("1550x800+0+0")
        #self.root.config(bg="lime")
        #img=PhotoImage('logo2.png')
...
#====================images===========================
        ...
        lbltitle=Label(self.root,text=" WELCOME  TO  MEDICAL  STORE",bd=8,relief=RIDGE,fg='red',bg="white",
                        font=("times new roman",35,"bold"),padx=2,pady=4)
        lbltitle.place(x=100,y=0,width=1330,height=100)

        img6=Image.open("log2.jpg")
...
#=======================Buttons in Dashboard=====================
        ...
        img3=Image.open("button1.png") #Admin Button
        img3=img3.resize((150,135),Image.ANTIALIAS)
        self.Photoimg3=ImageTk.PhotoImage(img3)
        b1=Button(self.root,image=self.Photoimg3,borderwidth=0,command=self.Login_Window)
        b1.place(x=100,y=150)

       
        img4=Image.open("button2.png") #User Button
        img4=img4.resize((150,135),Image.ANTIALIAS)
        self.Photoimg4=ImageTk.PhotoImage(img4)
        b1=Button(self.root,image=self.Photoimg4,borderwidth=0,command=self.User_Login_Window)
        b1.place(x=100,y=350)

        img5=Image.open("button3.png") #Exit
        img5=img5.resize((150,135),Image.ANTIALIAS)
        self.Photoimg5=ImageTk.PhotoImage(img5)
        b1=Button(self.root,image=self.Photoimg5,borderwidth=0,command=self.root.destroy)
        b1.place(x=100,y=550)



       
#=================linking to login Window==============
      def Login_Window(self):
        self.newWindow = Toplevel(self.root)
        self.app = Window2(self.newWindow)


#================linking to Admin Window================        
      def Admin_Login_Window(self):
        self.newWindow = Toplevel(self.root)
        self.app = Window3(self.newWindow)

#===============linking to user Window==================
...
#======================Frames=====================================
        ...
        self.btnAdminRegistration=Button(self.LoginFrame3, text="Admin Login",width=16,
    ...
    def Login_System(self):
        ...
        if(user=="Admin" and pas=="Admin@123"):
           ...
           #self.btnAdminRegistration.config(state=NORMAL)
            ...
            messagebox.askyesno("Login System","too Bad,Invaild login detail",parent=self.root)
    ...
    def Reset(self):
         ...
         self.txtUsername.focus()
    def iExit(self):
        self.iExit = messagebox.askyesno("Login System","confirm if you want to exit",parent=self.root)
        if self.iExit>0:
            self.root.destroy()
        else:
...
class Window3:
      def __init__(self, root):
           ...
           # DataFrameRight.place(x=910,y=5,width=550,height=350)
            ...
            btnExitMed=Button(ButtonFrame,text="EXIT",command=self.root.destroy,font=("arial",13,"bold"),width=14,bg="red",fg="white")
            ...
            comrefno.bind("<<ComboboxSelected>>",self.reftomadn)
            ...
            txtMedicineName=Entry(DataFrameLeft,textvariable=self.medname_var,width=22,bg="white",bd=2,relief=RIDGE,font=("arial",12,"bold"))
      ...
      def UpdateMed(self):
            if self.refMed_var.get()=="" or self.addmed_var.get()=="" :  
                messagebox.showerror("Error","All Field are Requried",parent=self.root) 
            else:
                 conn=mysql.connector.connect(host="localhost",username="root",passwd="Vaishnavi@123",database="mydata",port=3306)
                 my_cursor=conn.cursor()
                 ...
                 messagebox.showinfo("Success","Medicine has be updated",parent=self.root)
      ...
      def DeleteMed(self):
            ...
            self.fetch_dataMed()
            conn.close()
            messagebox.showinfo("Success","Medicine has be Deleted",parent=self.root)
      ...
      def ClearMed(self):
            ...
            #================================================ medicine information Main table=================================================
      ...
      def add_data(self):
            if self.ref_var.get()=="":
                ...
                Price="\d+\.\d+$"
                def checkL(lot):
                    if(re.search(Lotno,lot)):
                       checkISD(self.issuedata_var.get())
                    else:
                        messagebox.showerror("Error"," Enter valid Lot no",parent=self.root)
                def checkISD(date):
                    if(re.search(Date,date)):
                        checkEXD(self.expdata_var.get())
                    else:
                        messagebox.showerror("Error"," Enter valid  Issue date",parent=self.root)
                def checkEXD(date):
                    if(re.search(Date,date)):
                        checkU(self.uses_var.get())
                    else:
                        messagebox.showerror("Error"," Enter valid Exp date",parent=self.root)
                def checkU(use):
                    if(re.search(Use,use)):
                        checkSE(self.sideEffect_var.get())
                    else:
                        messagebox.showerror("Error"," Enter valid Uses",parent=self.root)
                def checkSE(use):
                    if(re.search(Use,use)):
                        checkPW(self.warning_var.get())
                    else:
                        messagebox.showerror("Error"," Enter valid Side Effect",parent=self.root)
                def checkPW(use):
                    if(re.search(Use,use)):
                        checkD(self.dosage_var.get())
                    else:
                        messagebox.showerror("Error"," Enter valid prec &Warning",parent=self.root)
                def checkD(lot):
                    if(re.search(Lotno,lot)):
                       checkPR(self.price_var.get())
                    else:
                        messagebox.showerror("Error"," Enter valid Dosage",parent=self.root)
                def checkPR(price):
                    if(re.search(Price,price)):
                        checkPQ(self.product_var.get())
                    else:
                        messagebox.showerror("Error"," Enter valid Price",parent=self.root)
                ...
                def checkPQ(lot):
                    if(re.search(Lotno,lot)):
                        ...
                        #self.get_cursor()
                        conn.close()
                        messagebox.showinfo("Success","data has be inserted",parent=self.root)

                    else:
                        messagebox.showerror("Error"," Enter valid Prodect & Qty",parent=self.root)
                
                if(self.lot_var.get()==""):
                    messagebox.showerror("Error"," Enter valid Lot NO",parent=self.root)
                else:
                    checkL(self.lot_var.get())
      ...
      def Update(self):
            if self.ref_var.get()=="" or self.lot_var.get()=="" :
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("UPDATE","Records has be Updated Successfully",parent=self.root)  
      ...
      def delete(self):
            ...
            self.fetch_data()
            conn.close()
            messagebox.showinfo("Delete","Info deleted successfully",parent=self.root)
...
#================== RefNO to medicine name=============
      ...
      def search_data(self):
            if self.search_var.get()=="" or self.searchTxt_var.get()=="":
                messagebox.showerror("Error","vaild  data")
            else:
                try:
                    conn=mysql.connector.connect(host="localhost",username="root",passwd="Vaishnavi@123",database="mydata",port=3306)
                    my_cursor=conn.cursor()
...
class Window4:
      def __init__( self , root ):
            ...
            r=my_cursor.fetchall()
            self.SubCategoryTab=r  #refno in r
            a=0
            for i in r:
                if(i[0]=="Liquid"):
                    li.append(i[1])
    ...
    #===============================Product LabelFrame=========================
            ...
            #category means Type of medicine
           ...
           #Subcategory means Ref NO
            ...
            self.ComboQty=ttk.Entry(Product_Frame,textvariable=self.Qty,font=("times new roman",10,"bold"),width=24)
            ...
            if self.ComboProduct.get()=="":
                ...
                def checkE(email):
                    if(re.search(regex,email)):   
                        ...
                        messagebox.showerror("Error","Invalid Email",parent=self.root) #print("Invalid Email")
                def checkN(name):
                    if(re.search(Name,name)):
                        checkE(self.txtEmail.get())
                    else:
                        messagebox.showerror("Error"," Enter valid Name",parent=self.root)
                    
                if(self.c_phon_var.get()=="" or len(self.c_phon_var.get())!=10):
                    messagebox.showerror("Error"," Enter valid contact NO",parent=self.root)
                else:
                    if self.c_name_var.get()=="":
                        messagebox.showerror("Error","Enter vaild Name",parent=self.root)
                    else:
                        checkN(self.txtCustName.get())             
      ...
      def save_bill(self):
            ...
            if op>0:
                ...
                op=messagebox.showinfo("Saved",f"Bill No:{self.bill_no_var.get()} saved successfully",parent=self.root)
                f1.close()
      ...
      def find_bill(self):
            ...
            for i in os.listdir("bills/"):
                if i.split('.')[0]==self.txt_Entry_Serach.get():
                    ...
                    found="yes"
            if found=="no":
                 messagebox.showerror("Error","not found bill no",parent=self.root)

      def clear(self):
            self.textarea.delete(1.0,END)
            
            self.c_name_var.set("")
            self.c_phon_var.set(0)
            self.c_mail_var.set("") 
            self.bill_no_var=IntVar()
           ...
           # self.ComboQty.set(0)
            ...
            #================================================functions=================================================
      def categories(self,event=""): #Type of Medicine
